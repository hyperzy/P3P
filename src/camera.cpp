...
void ImageBase::setRotation(const Mat3 &R) {
    __rotation = R;
    r_set = true;
}

void ImageBase::setTranslation(const Vec3 &t) {
    __translation = t;
    t_set = true;
}

void ImageBase::saveRotation(ofstream &fout) const {
    if (!r_set) {
        cerr << "Rotation matrix is empty" << endl;
        exit(EXIT_FAILURE);
    }
    fout << __rotation << "\n";
}

void ImageBase::saveTranslation(ofstream &fout) const {
    if (!t_set) {
        cerr << "Translation vector is empty" << endl;
        exit(EXIT_FAILURE);
    }
    fout << __translation << "\n";
}
...
void DepthImage::loadKinect(const std::string &filepath) {
    this->__max_inf = false;
    try {
        this->__img = imread(filepath, cv::IMREAD_ANYDEPTH);
        if (!this->__img.data) {
            throw runtime_error("Invalid Image Path.");
        }
        double min_v;
